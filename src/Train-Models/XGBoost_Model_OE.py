...
data['OE'] = list2
OE = data['OE']
data.drop(['Score', 'Home-Team-Win', 'TEAM_NAME', 'Date', 'TEAM_NAME.1', 'Date.1', 'OU-Cover', 'OU', "OE"], axis=1, inplace=True)
...
for x in tqdm(range(300)):
    x_train, x_test, y_train, y_test = train_test_split(data, OE, test_size=.1)
