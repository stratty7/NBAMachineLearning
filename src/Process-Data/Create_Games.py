...
for key, value in config['create-games'].items():
    ...
    for row in odds_df.itertuples():
        ...
        #row[5] is spread
            ...
            winning marging = 5 
            away spread = 3
            if (winning margin - away speard)  > 0
                0
            else
                1
                
            spread = 4.5 maring = 5   =  5 - 4.5 = 0.5  N
            spread = -4.5 margin = 5 = 5 - -4.5 =    9.5 n                                                                                                                                                                                          .5 margin = 5  =  5 - -4.5 = 9.5 N
            speard = 4.5 margin = -5   = -5 - 4.5 = -9.5 Y
            spread = -4.5 margin = -5 = -5 - -4.5 = -0.5 y
            
            
            """
            try:
                if (row[9] - float(row[5])) > 0:
                    spread_cover.append(0)
                else:
                    spread_cover.append(1)
            except:
                spread_cover.append(0)
...
frame['Spread-Cover'] = np.asarray(spread_cover)
